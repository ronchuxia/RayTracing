...
public:
  uniform_scale(shared_ptr<hittable> _object, double _scale)
    : object(_object), scale(_scale)
  {
  ...
  bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
      ...
      // Determine whether an intersection exists along the original ray (and if so, where)
      if (!object->hit(scaled_r, ray_t, rec))
          return false;

      // Scale the intersection back
      rec.p *= scale;
